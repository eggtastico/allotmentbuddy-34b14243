...
export function BedInfoPanel({
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="max-h-[85vh] overflow-y-auto w-full sm:max-w-md p-0">
        {/* Header */}
        ...
        <div className="px-4 py-2 text-xs text-muted-foreground space-y-1 border-b border-border/50">
          <div><span className="font-medium">Type:</span> {catalogueName}</div>
          <div><span className="font-medium">Size:</span> {bed.widthCells} × {bed.heightCells} cells</div>
          <div><span className="font-medium">Plants:</span> {bedsInBed.length}</div>
        </div>
          ...
          <table className="w-full text-xs">
            <tbody>
              {rotationTable.map((row, i) => (
                <tr key={i} className={row.source === 'current' ? 'bg-primary/10 font-semibold' : ''}>
